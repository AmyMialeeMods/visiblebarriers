        ...
        if (VisibleBarriers.isVisibilityEnabled()) {
            if (entity instanceof AreaEffectCloudEntity cloud) {
                if (!this.floater.getItem().isOf(Items.LINGERING_POTION)) {
                    ...
                    potion.apply(DataComponentTypes.POTION_CONTENTS, PotionContentsComponent.DEFAULT, c -> new PotionContentsComponent(c.potion(), cloud.potionContentsComponent.customColor(), c.customEffects()));
